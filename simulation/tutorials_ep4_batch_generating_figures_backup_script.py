...
class Simulation_Benchmark:
    def __init__(self, d):
        ...
        def now_lets_generate_some_figures():
            ...
            fig, axes = plt.subplots(nrows=3, ncols=1, dpi=150, facecolor='w', figsize=(8,6), sharex=True)

            ax = axes[0]
            ax.plot(global_machine_times, global_CTRL_cmd_rpm)
            ax.plot(global_machine_times, global_CTRL_omega_r_mech)
            ax.set_ylabel(r'Speed [r/min]', multialignment='center', fontdict=font)

            ax = axes[1]
            ax.plot(global_machine_times, ZZZZ)
            ax.plot(global_machine_times, global_CTRL_idq_1_)
            ax.set_ylabel(r'$i_q$ [A]', multialignment='center', fontdict=font)

            ax = axes[2]
            ax.plot(global_machine_times, global_ualfa)
            ax.plot(global_machine_times, global_ubeta)
            ...
            fig.savefig(f'Shizhou-figures.pdf', dpi=400, bbox_inches='tight', pad_inches=0)
        now_lets_generate_some_figures()

        plt.figure(101, figsize=(15,4))
        plt.plot(gdd['CTRL.cmd_rpm']); plt.plot(gdd['CTRL.omega_r_mech']); plt.plot(gdd['CTRL.xS[1]'])
        plt.show()
...
'DC_BUS_VOLTAGE': 150,
'CL_SERIES_KP': 0.737168,
'CL_SERIES_KI': 214.011,
'VL_SERIES_KP': 0.323363,
'VL_SERIES_KI': 30.5565,
...
if __name__ == '!__main__':
    ...
    # basic settings
    CL_TS      = 1e-4 # 0.5e-4 # [sec]
    TIME_SLICE = 0.1  # [sec]
    NUMBER_OF_SLICES = 10
    VL_EXE_PER_CL_EXE = 5

    # init
    CTRL = The_Motor_Controller(CL_TS = CL_TS,
                                VL_TS = VL_EXE_PER_CL_EXE*CL_TS,
                                init_npp = 21,
                                init_IN = 72/1.414,
                                init_R = 0.1222,
                                init_Ld = 0.000502,
    ...
    CTRL.bool_apply_sweeping_frequency_excitation = False
    CTRL.bool_overwrite_speed_commands = True
    ACM       = The_AC_Machine(CTRL, MACHINE_SIMULATIONs_PER_SAMPLING_PERIOD=500)
    ...
    # unpack global data variable
    global_CTRL_cmd_rpm   , \
    global_CTRL_omega_r_mech   , \
    global__OB_speed   , \
    global________x5   , \
    global___KA        , \
    global____ACM_id   , \
    ...
    plt.figure(101, figsize=(15,4))
    plt.plot(global_CTRL_cmd_rpm); plt.plot(global_CTRL_omega_r_mech); plt.plot(global__OB_speed)

    plt.figure(2, figsize=(15,4)); #plt.ylim([-1e-1, 1e-1])
    plt.plot( (global_CTRL_cmd_rpm - global_CTRL_omega_r_mech) ) # [2000:5000]
    ...
    plt.plot( ZZZZ); plt.plot( global_CTRL_idq_1_)
