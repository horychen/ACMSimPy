#!/usr/bin/env python
"""
Test: Stage 3 with NSO using ENCODER angle for Park transform
(decouple NSO from AF angle to break positive feedback loop)
"""
...
from demo_sensorless_active_flux import (wrap_angle, angle_error, angle_diff_scalar,
    NaturalSpeedObserver)
from eval_staged_tuning import get_motor_preset


def run_decoupled_s3(d, CLBW_Hz, zeta, af_Kp, af_Ki, R_mis, v_off,
                     omega_ob, cmd_rpm, load_step,
                     nso_uses_encoder_angle=False, verbose=True):
    """
    Stage 3 sensorless, but NSO Park transform can use:
    - AF angle (default, coupled)  
    - Encoder angle (decoupled test)
    """
    dd = copy.deepcopy(d)
    ...
    # Full sensorless: AF angle + NSO speed
    CTRL.index_separate_speed_estimation = 1
    CTRL.ell1 = CTRL.ell2 = CTRL.ell3 = CTRL.ell4 = 0.0
    CTRL.bool_use_sensorless_theta = 1  # Use AF angle for Park
    ...
    th_af = np.zeros(n_ctrl); w_nso = np.zeros(n_ctrl); cmd_tr = np.zeros(n_ctrl)
    ...
    for sl in range(int(total_time / CL_TS)):
        ...
        if ci > 0:
            CTRL.vartheta_d = th_af[ci-1]  # AF angle → Park transform
            CTRL.xS[1] = nso.omega_elec     # NSO speed → speed loop
            # No torque FF
        ...
        psi_err = KE - af_amp
        af_corr_int += af_Ki * psi_err * CL_TS
        if af_amp > 1e-10:
            un_a, un_b = af_a/af_amp, af_b/af_amp
        else:
            un_a, un_b = 1.0, 0.0
        ...
        # NSO: choose Park transform angle
        if nso_uses_encoder_angle:
            # Use TRUE encoder angle (decoupled from AF)
            park_angle = th_true[ci]
        else:
            # Use AF angle (standard coupling)
            park_angle = theta_af_now

        cos_th = np.cos(park_angle); sin_th = np.sin(park_angle)
        id_m = ia*cos_th + ib*sin_th
        iq_m = -ia*sin_th + ib*cos_th
        uq_m = -ua*sin_th + ub*cos_th
        nso.step(iq_m, uq_m, id_m)
        w_nso[ci] = nso.omega_rpm

        ci += 1

    n = ci
    elapsed = _time.time()
    t = t_tr[:n]; ss = t > t[-1]*0.75
    ...
    return {'t': t, 'theta_true': th_true[:n], 'omega_true': w_true[:n],
            'theta_af': th_af[:n], 'omega_nso': w_nso[:n], 'cmd': cmd_tr[:n],
            'ang_rms': ang, 'trk_rms': trk, 'n_pp': n_pp}

# ============ MAIN ============
print('='*70)
print('  Decoupled NSO Park Transform Test on servo (ideal params)')
print('='*70)
...
configs = [
    # (nso_encoder, CLBW, label)
    (False, 50,  'coupled CL50'),
    (False, 100, 'coupled CL100'),
    (False, 200, 'coupled CL200'),
    (False, 500, 'coupled CL500'),
    (True,  50,  'DECOUPLED CL50'),
    (True,  100, 'DECOUPLED CL100'),
    (True,  200, 'DECOUPLED CL200'),
    (True,  500, 'DECOUPLED CL500'),
]

header = f'{"Label":<20s} | {"AngSS":>7s} {"TrkSS":>7s}'
print(header); print('-' * 40)

best_res = None; best_ang = 999; best_label = ''
for enc, clbw, label in configs:
    r = run_decoupled_s3(
        p['d'], CLBW_Hz=clbw, zeta=10,
        af_Kp=p['af_kp'], af_Ki=p['af_ki'],
        R_mis=1.0, v_off=(0,0), omega_ob=80,
        cmd_rpm=cmd, load_step=p['load_step'],
        nso_uses_encoder_angle=enc, verbose=False)
    flag = ' ★' if r['ang_rms'] < 5 else ''
    print(f'{label:<20s} | {r["ang_rms"]:>7.2f} {r["trk_rms"]:>7.2f}{flag}')
...
# Plot best decoupled
if best_res is not None and best_ang < 30:
    fig, axes = plt.subplots(3, 1, figsize=(14, 9), sharex=True)
    t = best_res['t']
    axes[0].plot(t, best_res['cmd'], 'r--', alpha=0.5, label='cmd')
    axes[0].plot(t, best_res['omega_true'], 'b', label='true')
    axes[0].plot(t, best_res['omega_nso'], 'g', alpha=0.7, label='NSO')
    axes[0].set_ylabel('Speed [rpm]'); axes[0].legend(); axes[0].set_title(f'{best_label} | Ang={best_ang:.2f} deg')
    ...
    plt.tight_layout(); plt.savefig('fig_decoupled_servo.png', dpi=150); plt.close()
    print('Saved fig_decoupled_servo.png')
